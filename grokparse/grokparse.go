...
func expandPattern(p string, seen map[string]struct{}) string {
    ...
    for {
        ...
        if _, already := seen[key]; already {
            p = re.ReplaceAllString(p, ".*"); break }
        seen[key] = struct{}{}
        sub, ok := Patterns[key]
        if !ok { p = re.ReplaceAllString(p, ".*")
        } else { sub = expandPattern(sub, seen)
...
func CompileGrok(pattern string) (*regexp.Regexp, error) {
    ...
    result := re.ReplaceAllStringFunc(pattern, func(s string) string {
        ...
        if m[2] != "" { return fmt.Sprintf("(?P<%s>%s)", m[2], base) }
        return fmt.Sprintf("(%s)", base)
    })
    return regexp.MustCompile(result), nil
}
func ParseLine(pattern, logline string) (map[string]string, error) {
    re, err := CompileGrok(pattern)
    if err != nil { return nil, err }
    match := re.FindStringSubmatch(logline)
    if match == nil { return nil, fmt.Errorf("no match") }
    out := map[string]string{}
    for i, name := range re.SubexpNames() {
        if i > 0 && name != "" { out[name] = match[i] }
    }
    return out, nil
}
